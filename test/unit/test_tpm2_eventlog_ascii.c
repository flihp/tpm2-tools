...
static void dump_bytes_toolong(void **state) {

    (void)state;

    assert_false(dump_bytes(NULL, 0, MAX_LINE_LENGTH, 5));
}
static void get_alg_name_sha256(void **state){

    (void)state;

    assert_string_equal(get_alg_name(TPM2_ALG_SHA256), "TPM2_ALG_SHA256");
}
static void get_alg_name_sha384(void **state){

    (void)state;

    assert_string_equal(get_alg_name(TPM2_ALG_SHA384), "TPM2_ALG_SHA384");
}
static void get_alg_name_sha512(void **state){

    (void)state;

    assert_string_equal(get_alg_name(TPM2_ALG_SHA512), "TPM2_ALG_SHA512");
}
...
static void test_ascii_tpm2_event_header_bad(void **state) {
    ...
    assert_false(ascii_tpm2_event_header(&event_hdr, 5));
}
static void test_ascii_tpm2_event_header(void **state) {
    ...
    assert_true(ascii_tpm2_event_header(&event_hdr, sizeof(event_hdr)));
}


static void test_ascii_tpm2_digest_callback(void **state){
    ...
    assert_true(ascii_tpm2_digest_callback(digest, TPM2_SHA1_DIGEST_SIZE, &digestcb_data));
}
static void test_ascii_tpm2_eventbuf_nosize(void **state) {
    ...
    event->EventSize = 6;
    assert_false (ascii_tpm2_eventbuf(event, sizeof(*event) - 1));
}
static void test_ascii_tpm2_eventbuf_nobuf(void **state) {
    ...
    event->EventSize = 6;
    assert_false(ascii_tpm2_eventbuf(event, sizeof(*event) + 3));
}
static void test_ascii_tpm2_eventbuf(void **state) {
    ...
    event->EventSize = 6;
    assert_true(ascii_tpm2_eventbuf(event, sizeof(*event) + 6));
}
static void test_ascii_event_callback(void **state){
    ...
    assert_true(ascii_event_callback(eventhdr, sizeof(buf), &count));
}

int main(void) {

    const struct CMUnitTest tests[] = {
        cmocka_unit_test(dump_bytes_toolong),
        cmocka_unit_test(get_alg_name_sha256),
        cmocka_unit_test(get_alg_name_sha384),
        cmocka_unit_test(get_alg_name_sha512),
        ...
        cmocka_unit_test(test_ascii_tpm2_event_header_bad),
        cmocka_unit_test(test_ascii_tpm2_event_header),
        cmocka_unit_test(test_ascii_tpm2_eventbuf_nosize),
        cmocka_unit_test(test_ascii_tpm2_eventbuf_nobuf),
        cmocka_unit_test(test_ascii_tpm2_eventbuf),
        cmocka_unit_test(test_ascii_tpm2_digest_callback),
        cmocka_unit_test(test_ascii_event_callback),
    };
